...
| Rule | Detail |
|------|--------|
| Syntax | Wrap names in curly braces `{ }` |
| Name matching | Imported name must match the exported name exactly |
| Tree-shaking | Import only what you need — unused exports get dropped |

### Rename with `as`

Two files both export `BASE_URL`. Name clash. Fix = alias with `as`:

| Original import | Aliased as | Value |
|----------------|------------|-------|
| `BASE_URL` from utils.js | `bul_util` | `https://api.staging.com` |
| `BASE_URL` from testutils.js | `bul_testtul` | `https://app.vwo.com` |
...
| Rule | Detail |
|------|--------|
| Syntax | No curly braces — `import x from` |
| Naming | Pick any name on import — not tied to original |
| Cardinality | Only **one** default export allowed per file |
...
| Scenario | Approach |
|----------|----------|
| Many helpers in a module | Use **named exports** |
| One clear "main" thing the module IS | Use **default export** |
| Name collision across modules | Alias named imports with `as` |
| File has a main thing + extras | Do both — default is headline, named exports are extras (see `logger.js`) |
